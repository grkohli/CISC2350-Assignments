...
<body>
	<?php
		...
		$students = array(
			/*
				YE
				you modified the data array -2pts
			 */
			'Rick' => 124,
			'Morty' => 24,
			'Summer' => 48,
	 ...
	 /*
	 	YE
	 	great function! please make sure you are adding comments
	  */
	...
	function get_grad_credits( $earned_credits){
		...
		/*
			YE
			if you are returning the value, you don't need to echo
			it in the function.  Just echo the return value when it is
			returned
		 */
			...
			echo "<p>Credits I Currently Have == $earned_credits</p>";
		 		...
		 		if($earned_credits >= 0 && $earned_credits <= $freshmancredits){
					...
					$nextclassification = "Sophomore";
		 			...
		 			/*
		 				YE
		 				there can only be 1 return statement.  You need to use a specific datastructure
						to return both the credits and the classification -5pts
		 			 */
		 			return( $credits_needed);
	...
	<section>
		...
		foreach( $students as $name => $earned_credits) {
			...
			/*
				YE
				you are in a forech loop, no need for a counter -2pts
			 */
			$count++;
		}
		...
		/*
			YE
			did not use built in functions to format names -10pts
		 */
		?>
	</section>
